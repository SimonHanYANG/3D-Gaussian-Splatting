...
class Gaussian3ds(nn.Module):
    ...
    def _tocpp(self):
        # 将 Gaussian3ds 对象的属性复制到一个C++对象中，可能用于与 C++ 实现的库或函数交互
    ...
    def to(self, *args, **kwargs):
        # 重载torch.nn.Module的to方法，使得Gaussian3ds对象可以被移动到指定的设备（CPU或GPU）上，同时保证其所有属性也被适当地转移
    ...
    def filte(self, mask):
        # 根据给定的掩码过滤高斯模型，返回一个新的Gaussian3ds实例，仅包含掩码为True的元素。这可以用于剔除不必要的模型或选择特定的子集。
    ...
    def get_gaussian_3d_cov(self, scale_activation="abs"):
        # 根据高斯模型的尺度和旋转计算其3D协方差矩阵。根据尺度激活函数（scale_activation），尺度可以是绝对值或指数形式
        R = q2r(self.quat)
    ...
    def reset_opa(self):
        # 重置高斯模型的不透明度为一个固定值，通常用于初始化或重新初始化模型的可见性。
    ...
    def adaptive_control(
        ...
        # 是否进行 adaptive control 的阈值 0.0002 in paper
        grad_thresh=0.0002,
        # 梯度聚合方法，用于决定如何从梯度信息中提取密集化信号
        grad_aggregation="max",
        # 是否使用克隆或分裂策略进行密集化
        use_clone=True,
        use_split=True,
        
        # 克隆位移的大小，决定了克隆生成的新高斯模型相对于原模型的位移
        clone_dt=0.01,
    ):
        # grad: B x 3
        # densification
    ...
    def project(self, rot, tran, near, jacobian_calc, scale_activation="abs"):
        # 将3D高斯模型投影到2D图像平面上，包括计算图像空间中的位置、颜色、不透明度和2D协方差。这是渲染过程的一部分，将3D模型转换为2D图像表示
...
class Splatter(nn.Module):
    ...
    def parse_imgs(self):
        # 从 images.bin 中读取所有 imgs 的信息，信息来源 images/ 不是 images_4/
        ...
        for img_id in tqdm(img_ids):
            ...
            # 得到的还是 images 中的图片名称，所以 images_4 中的名称必须和 images 中的图片相符
